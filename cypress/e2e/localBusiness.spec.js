...
describe('Local Business JSON-LD', () => {
  it('matches schema', () => {
    cy.visit('http://localhost:3000/jsonld/localBusiness/localBusiness');
    cy.get('head script[type="application/ld+json"]').then(tags => {
      const jsonLD = JSON.parse(tags[0].innerHTML);
      assertSchema(schemas)('Local Business', '1.4.0')(jsonLD);
    });
  });
  ...
  it('renders with single address in string', () => {
    cy.visit('http://localhost:3000/jsonld/localBusiness/singleAddressArray');
    cy.get('head script[type="application/ld+json"]').then(tags => {
      const jsonLD = JSON.parse(tags[0].innerHTML);
      expect(jsonLD).to.deep.equal({
        '@context': 'https://schema.org',
  ...
  it('renders with multiple addresses', () => {
    cy.visit('http://localhost:3000/jsonld/localBusiness/multipleAddresses');
    cy.get('head script[type="application/ld+json"]').then(tags => {
      const jsonLD = JSON.parse(tags[0].innerHTML);
      expect(jsonLD).to.deep.equal({
        '@context': 'https://schema.org',

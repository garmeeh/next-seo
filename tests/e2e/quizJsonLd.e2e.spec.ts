...
test.describe("QuizJsonLd", () => {
  ...
  test("properly escapes special characters in Quiz content", async ({
    page,
  }) => {
    // Create a test page with special characters
    await page.route("/quiz-special-chars", async (route) => {
      await route.fulfill({
        status: 200,
        ...
        body: `
          ...
          <html>
            <head>
              <script type="application/ld+json">
              {
                ...
                "hasPart": [{
                  "@type": "Question",
                  "eduQuestionType": "Flashcard",
                  "text": "What is 5 < 10 && 10 > 5?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "True - both conditions are satisfied"
                  }
                }]
              }
              </script>
            ...
            <body>
              <h1>Quiz with Special Characters</h1>
            </body>
          </html>
        `,
      });
    ...
    await page.goto("/quiz-special-chars");

    const jsonLdScript = await page
      .locator('script[type="application/ld+json"]')
      .textContent();

    // Should be able to parse despite special characters
    const jsonData = JSON.parse(jsonLdScript!);
    expect(jsonData.hasPart[0].text).toContain("What is 5 < 10");
  });
});

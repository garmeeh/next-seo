...
test.describe("OrganizationJsonLd", () => {
  ...
  test("properly escapes HTML entities in organization content", async ({
    page,
  }) => {
    // Create a test page with HTML entities in content
    await page.route("/organization-html-entities", async (route) => {
      await route.fulfill({
        status: 200,
          ...
          <html>
            <head>
              <script type="application/ld+json">
              {
                ...
                "@type": "Organization",
                "name": "Test & Co <script>alert('xss')</script>",
                "description": "We make widgets & gadgets"
              }
              </script>
            </head>
            <body>Test</body>
    ...
    await page.goto("/organization-html-entities");

    // Verify that the JSON-LD script exists and contains properly escaped content
    const jsonLdScript = await page
      .locator('script[type="application/ld+json"]')
      .textContent();
    expect(jsonLdScript).toBeTruthy();

    // The content should be valid JSON (properly escaped)
    const jsonData = JSON.parse(jsonLdScript!);
    expect(jsonData.name).toContain("Test & Co");
    expect(jsonData.description).toBe("We make widgets & gadgets");

    // Ensure no script tags are executed
    const alerts: string[] = [];
